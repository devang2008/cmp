  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1">End-to-end encrypted conversations with buyers</p>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-slate-700/30">
            {deals.map((deal: any) => (
              <Link key={deal.id} href={`/deal/${deal.id}/chat`}
                ...
                <div className="w-11 h-11 rounded-full bg-gradient-to-br from-purple-500 to-cyan-600 flex items-center justify-center text-white text-xs font-bold shrink-0">
                  {deal.buyer_alias?.charAt(0)?.toUpperCase()}{deal.buyer_alias?.split("-")[1]?.charAt(0)?.toUpperCase() || ""}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-white font-mono truncate">{deal.buyer_alias}</span>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-slate-400 max-w-xs">
              Submit a proposal and get it accepted to start an encrypted conversation.
            </p>

...
export default function VendorDealsPage() {
  ...
  const DealCard = ({ deal }: { deal: any }) => (
    <Link href={`/dashboard/vendor/deals/${deal.id}`}
      ...
      <div className="flex items-center justify-between mb-2">
        <span className="font-mono text-cyan-400 text-sm">{deal.buyer_alias}</span>
  ...
  if (isLoading) return <div className="animate-pulse space-y-4">{[...Array(3)].map((_, i) => <div key={i} className="h-24 bg-slate-800/50 rounded-xl" />)}</div>
    ...
    <div className="space-y-8">
      ...
      {(!deals || deals.length === 0) && (
        <div className="text-center py-16 bg-slate-800/30 rounded-xl border border-slate-700/50">
          ...
          <p className="text-slate-400">Get a proposal accepted to start your first deal.</p>
        </div>
      )}
    </div>
  )

// POST /api/email/send — Internal email dispatch with templates
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ data: null, error: error.message, sent: false }, { status: 200 })
    }

    return NextResponse.json({ data: { id: data?.id }, error: null, sent: true }, { status: 200 })
  } catch (e: unknown) {

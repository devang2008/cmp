// GET /api/cmp/buyer/proposals/[req_id] — proposals for a specific requirement
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ req_id: string }> }) {
  try {
    ...
    // Enrich with trust score from Prisma (alias_directory)
    if (proposals.length > 0) {
      const aliases = Array.from(new Set(proposals.map(p => p.vendor_alias)))
      
      const aliasData = await prisma.aliasDirectory.findMany({

// GET/PATCH/DELETE /api/cmp/buyer/requirements/[id]
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    // Run auth + MongoDB + params in parallel (saves ~400ms)
    const [profile, col, { id }, propCol] = await Promise.all([
      requireAuth(),
    ...
    // Get proposals for this requirement (propCol already fetched in parallel)
    const proposals = await propCol.find({ requirement_id: id }).sort({ created_at: -1 }).toArray()

    // Enrich proposals with vendor ratings from Prisma (alias_directory)
    ...
    if (vendorAliases.length > 0) {
      const directories = await prisma.aliasDirectory.findMany({
        where: { alias: { in: vendorAliases } },
        select: {
          alias: true,

// GET /api/vendor/proposals — my proposals
// POST /api/vendor/proposals — submit proposal
...
export async function POST(request: NextRequest) {
  try {
    ...
    // --- NEW: Internal Email Dispatch ---
    try {
      // Try resolving buyer email
      const { data: buyerProfile } = await supabase
        .from('profiles').select('id').eq('alias', req.alias).single()
    ...
    try {
      await supabase.functions.invoke('compute-trust', { body: { alias: profile.alias } })
    } catch (trustErr) {
